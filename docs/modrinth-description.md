...
### 🔦 Cheese highlight
- The cheese glows through walls (configurable color) and can be **rendered up to 5× bigger** so you actually spot it in time.
- Detection works out of the box (the cheese head texture is built in), with heuristics and a `/ratcheese scan` debug tool as a fallback.
...
- Client-side only, parses only your own chat messages, automates nothing.
...
Клиентский мод для пета **Крыса** (Rat) на Hypixel Skyblock: подсветка и увеличение сыра (живёт всего ~10 секунд!), HUD с баффами Magic Find и таймерами, счётчик крысиного лута, алерты о сыре и 24 ачивки с секретками. Настройки — `/ratcheese` или через Mod Menu, русская локализация в комплекте.
